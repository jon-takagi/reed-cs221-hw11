          ...
          unsigned nthread = 1)
{
  ...
  auto run_one_thread = [&]() {
      // auto thread_best = ga_search(cities, iters / nthread, pop_size, mutation_rate);
      auto thread_best = Cities::permutation_t(cities.size());
      TournamentDeme deme(&cities, pop_size, mutation_rate);

      // Evolve the population to make it fitter and keep track of
      // the shortest distance generated
      for (long i = 1; i <= iters/nthread/pop_size; ++i) {
        deme.compute_next_generation();    // generate next generation

        // Find best individual in this population
        const auto ordering = deme.get_best()->get_ordering();
        if (is_improved(cities, ordering, best_dist, i * pop_size)) {
          thread_best = ordering;
        }
      }
      if(cities.total_path_distance(thread_best) < cities.total_path_distance(best_ordering)) {
          auto guard = std::lock_guard(best_mutex);
      // Repeat check, maybe something changed:
          if (cities.total_path_distance(thread_best) < cities.total_path_distance(best_ordering)) {
            best_ordering = thread_best;
            best_dist = cities.total_path_distance(thread_best);
          }
      }
  };
...
int main(int argc, char** argv)
{
  ...
  constexpr unsigned NUM_ITER = 5'000'000;
